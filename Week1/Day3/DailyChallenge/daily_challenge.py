...
def letter_index_dictionary():
    ...
    while word is None:
        try:
            word = input(f"Enter a word of your liking: ").strip()
            if not word.isalpha():
                print(f"Please enter a valid word")
                word = None
        except ValueError:
            print(
                "Invalid input. Please enter a word only consisting alphabetical characters."
            )

    for i in range(len(word)):
        if word[i] not in char_index_dict:
            char_index_dict[word[i]] = [i]
        else:
            char_index_dict[word[i]].append(i)
...
def clean_data(items_purchase: dict, wallet: str):
    cleaned_dict = {
        key: int(value.replace("$", "").replace(",", ""))
        for key, value in items_purchase.items()
    }

    cleaned_wallet = int(wallet.replace("$", "").replace(",", ""))
    return cleaned_dict, cleaned_wallet


def affordable_items(items_purchase: dict, wallet: str):
    basket = []
    cleaned_dict, cleaned_wallet = clean_data(items_purchase, wallet)

    for item, price in cleaned_dict.items():
