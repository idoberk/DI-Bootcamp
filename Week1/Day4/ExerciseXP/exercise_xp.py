...
def generate_and_compare(number: int):
    random_int = random.randint(1, 100)

    print(
        f"{'Success!' if random_int == number else 'Fail! Your number: ' + str(number) + ', Random number: ' + str(random_int)}"
    )


generate_and_compare(50)
...
make_shirt("medium")
make_shirt(text="This is awesome")
...
def show_magicians(magician_list: list):
...
def determine_season():
    ...
    while month is None:
        try:
            month = int(
                input("Enter an integer between 1 and 12 to determine the season: ")
            )
            if month < 1 or month > 12:
                print("Please enter a valid integer.")
                month = None
        except ValueError:
            print("Invalid input. Please enter a value between 1 and 12.")
...
def get_random_temp():
    ...
    return round(random.uniform(28, 40), 2)
...
def main():
    ...
    if random_temp < 16:
        print("It's really cold outside! Consider taking a sweater.")
    elif random_temp >= 16 and random_temp < 26:
        print("The temperature is very cozy.")
    else:
